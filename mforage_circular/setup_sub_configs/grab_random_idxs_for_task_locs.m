function [idxs, new_outer, new_inner] = grab_random_idxs_for_task_locs(outer, inner, n_locations_outer, n_locations_inner)

% first grab some indexes for the current task
inner = inner(~cellfun('isempty', inner));
these_inners = randperm(length(inner), n_locations_inner); % choose two quadrants from inner layer
these_outers = randperm(length(outer), n_locations_outer); % EC: choose 2 quadrants from outer layer
...
% now remove the selected indices from outer and inner
