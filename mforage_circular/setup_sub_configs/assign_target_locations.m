function [tasks] = assign_target_locations(i)
    ...
    thetas = [outer_thetas, inner_thetas]; % angles for outer and inner doors
    ...
    % THE BELOW IS CURRENTLY HARD CODED
    outer_idx = 1:4; % when you get your vector of target door locations, which 3 are from the outer ring?
    inner_idx = 5:6; % same as above but for inner
    n_locations_outer = 4;
    n_locations_inner = 2;
    [ta_idxs, nu_outer, nu_inner] = grab_random_idxs_for_task_locs(outer,...
        inner, n_locations_outer, n_locations_inner); % output is [taskA locs, remaining outer doors, remaining inner doors]
    ...
    for i_choose = 1:n_tasks_left_2choose
        legal = 0;
        while ~legal
            [idxs, ~, ~] = grab_random_idxs_for_task_locs(nu_outer, nu_inner, n_locations_outer, n_locations_inner);
    ...
    n_take_from_out = 2; % EC: need to change this
    n_take_from_in = 1;
